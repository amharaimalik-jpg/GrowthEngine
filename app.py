...
if "mined_data" in st.session_state:
    ...
    st.markdown("---")
    st.subheader("🔍 استعراض تفصيلي ونسخ التقارير للعملاء")
    
    df = st.session_state["mined_data"]
    for idx, row in df.iterrows():
        with st.expander(f"الفرصة #{idx+1}: {row['اسم المشروع / الفرصة']}"):
            st.write(f"**المالك / المسؤول:** {row['المالك / المسؤول']}")
            st.write(f"**رابط المشروع الحقيقي:** {row['رابط المشروع الحقيقي']}")
            st.text_area(f"التقرير الهندسي المقترح (الفرصة {idx+1})", row['التقرير الهندسي المقترح للتقديم'], key=f"proposal_{idx}")
